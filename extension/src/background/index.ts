...
import { initializeScheduler, handleAlarm, scheduleAutomation } from './scheduler';
import { executeAutomation, handleAutomationProgress, handleAutomationComplete } from './executor';
import { 
  getRecordingState, 
  setRecordingState, 
  clearRecordingState,
...
import type { MessageType, RecordedEvent, MousePosition, Automation, RecordingState } from '@/lib/types';
...
chrome.runtime.onMessage.addListener((message: MessageType | { type: string; [key: string]: unknown }, sender, sendResponse) => {
  console.log('[Simplest] Background received:', message.type);
  
  handleMessage(message, sender).then(sendResponse).catch((error: Error) => {
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, _tab) => {
  if (changeInfo.status === 'complete' && activeRecordingSession?.tabId === tabId) {
    ...
    try {
      ...
      console.error('[Simplest] Failed to restore panel after navigation:', error);
    }
  }
});
...
async function handleMessage(
  ...
  switch (message.type) {
    ...
    case 'STOP_RECORDING_WITH_NAME': {
      ...
      const name = ((message as unknown) as { name: string }).name || `Recording ${new Date().toLocaleString()}`;
      const tabId = activeRecordingSession.tabId;
      ...
      try {
        ...
        const response = await chrome.tabs.sendMessage(tabId, { type: 'STOP_RECORDING' }) as {
          success: boolean;
          events: RecordedEvent[];
          mouseMovements: MousePosition[];
          startUrl: string;
        ...
        recordingData = {
          events: [...activeRecordingSession.events, ...(response.events || [])],
          mouseMovements: [...activeRecordingSession.mouseMovements, ...(response.mouseMovements || [])],
    ...
    case 'RECORDING_RESUMED_ON_PAGE': {
      ...
      const resumeMsg = message as { type: string; sessionId?: string; eventCount?: number };
