...
import { createPanel, removePanel, updatePanelState, restorePanelState } from './panel';
import type { MessageType, Automation } from '@/lib/types';

console.log('[Simplest Automation] Content script loaded on:', window.location.href);
...
chrome.runtime.onMessage.addListener((message: MessageType | { type: string; [key: string]: unknown }, _sender, sendResponse) => {
...
window.addEventListener('message', (event) => {
  ...
  switch (event.data.type) {
    ...
    case 'SIMPLEST_RUN_SHARED':
      ...
      console.log('[Simplest] Received SIMPLEST_RUN_SHARED:', event.data.automationId);
...
async function handleMessage(message: MessageType | { type: string; [key: string]: unknown }): Promise<unknown> {
  switch (message.type) {
    case 'INJECT_PANEL': {
    ...
    case 'START_RECORDING': {
      const sessionId = (message as { sessionId?: string }).sessionId || crypto.randomUUID();
      startRecording(sessionId);
      updatePanelState({ mode: 'recording', eventCount: 0, duration: 0, startTime: Date.now() });
    ...
    case 'RESTORE_RECORDING_STATE': {
      ...
      const { sessionId, eventCount, duration, isPaused } = (message as unknown) as {
        sessionId: string;
        eventCount: number;
        duration: number;
        isPaused: boolean;
    ...
    case 'RUN_AUTOMATION': {
      ...
      replayAutomation(automation as Parameters<typeof replayAutomation>[0], (completed, total) => {
...
async function runSharedAutomation(automationId: string): Promise<void> {
  ...
  try {
    ...
    console.log('[Simplest] Fetching from:', `${API_BASE_URL}/api/automations/${automationId}`);
    const response = await fetch(`${API_BASE_URL}/api/automations/${automationId}`);
    ...
    console.log('[Simplest] Automation loaded:', automation.name, 'startUrl:', automation.startUrl);
    
    // Redirect to start URL and run there
    if (automation.startUrl && !window.location.href.startsWith(automation.startUrl.split('?')[0])) {
      // Store automation in session storage to run after redirect
      console.log('[Simplest] Storing automation and redirecting to:', automation.startUrl);
      sessionStorage.setItem('simplest_pending_automation', JSON.stringify(automation));
  ...
  if (match) {
    ...
    try {
      ...
      if (data.success && data.automation) {
        console.log('[Simplest] Fetched automation, executing:', data.automation.name);
        setTimeout(() => {
